...
import com.lin.dlivkfragment.service.OfflineMapService;
 ...
 * AMapV2地图中简单介绍离线地图下载
 */
public class OfflineMapActivity extends SwipeBackActivity implements
...
//		startService(new Intent(this,OfflineMapService.class));
		initDialog();
		...
		bindService(new Intent(this,OfflineMapService.class),connection,BIND_AUTO_CREATE);
		context = OfflineMapActivity.this;
...
//		expandableListView.setAdapter(adapter);
	...
	ServiceConnection connection = new ServiceConnection() {
		...
		public void onServiceConnected(ComponentName name, IBinder service) {
			offlineMapService = ((OfflineMapService.MyBind) service).getService();
//			provinceList = offlineMapService.getProvinceList();
//			cityMap = offlineMapService.getCityMap();
//			amapManager = offlineMapService.getAmapManager();
			OfflineMapService.MyBind myBind = (OfflineMapService.MyBind) service;
			provinceList = myBind.getProvinceList();
			cityMap = myBind.getCityMap();
			amapManager = myBind.getAmapManager();
			...
			myBind.setOfflineListener(OfflineMapActivity.this);
			initDialog.dismiss();
		}

		@Override
		public void onServiceDisconnected(ComponentName name) {
			offlineMapService.setOfflineListener(null);
		}
	};
	/**
	...
	private void initData() {
		...
		expandableListView.setOnChildClickListener(new OnChildClickListener() {
			...
			public boolean onChildClick(ExpandableListView parent, View v,
				...
				try {
					offlineMapService.downBegin();
					String name = cityMap.get(groupPosition).get(childPosition)
							.getCity();
					Log.d("OffineMapActivity","cityname:"+name);
...
//					Log.d("amap-onclick", name + " : " + url);
	...
	public void showDialog(final String name) {
		Builder builder = new Builder(OfflineMapActivity.this);
...
//					amapManager.pause();
	...
	OfflineMapCity aMapCity;
	/**
	 * 把一个省的对象转化为一个市的对象
	 */
	public OfflineMapCity getCicy(OfflineMapProvince aMapProvince) {
		if(aMapCity==null)aMapCity = new OfflineMapCity();
...
//			provinceList = offlineMapService.getProvinceList();
			...
			OfflineMapCity mapCity = cityMap.get(groupPosition).get(
					childPosition);
			Log.d("OfflineMapActivity","city:"+mapCity.getCity());
			// 通过getItem方法获取最新的状态
			if (groupPosition == 0 || groupPosition == 1 || groupPosition == 2) {
				...
				if (childPosition == 0) {
					// 省份
//					amapManager = offlineMapService.getAmapManager();
					Log.d("OfflineMapActivity", "mapCity:" + mapCity.getCity());
					OfflineMapProvince offlineMapProvince = amapManager.getItemByProvinceName(mapCity
							.getCity());
					mapCity = getCicy(offlineMapProvince);
				} else {
					// 城市
					mapCity = amapManager.getItemByCityName(mapCity.getCity());
				}
		...
		switch (status) {
			...
			case OfflineMapStatus.EXCEPTION_NETWORK_LOADING:
				Log.e("amap-download", "download: " + " EXCEPTION_NETWORK_LOADING "
						+ downName);
				Toast.makeText(OfflineMapActivity.this, "网络异常", Toast.LENGTH_LONG).show();
//			amapManager.pause();
				break;
